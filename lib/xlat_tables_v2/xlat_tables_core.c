...
static int mmap_add_region_check(const xlat_ctx_t *ctx, const mmap_region_t *mm)
{
	...
	for (const mmap_region_t *mm_cursor = ctx->mmap;
		 ...
		 */
        /*ERROR("mmap_add_region_check() fully_overlapped_va %d, mm_cursor->base_va %lx mm_cursor->base_pa %llx, base_va %lx, base_pa %llx\n", 
              fully_overlapped_va, mm_cursor->base_va, mm_cursor->base_pa, base_va, base_pa);
        ERROR("mmap_add_region_check() mm_cursor_end_va %lx mm_cursor->size %lx, end_va %lx, end_pa %llx\n", 
              mm_cursor_end_va, mm_cursor->size, end_va, end_pa);*/
