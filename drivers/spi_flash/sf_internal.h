...
int sst_write_wp(struct spi_flash *flash, uint32_t offset, size_t len,
		const void *buf);
int sst_write_bp(struct spi_flash *flash, uint32_t offset, size_t len,
		const void *buf);
#endif
...
struct spi_flash {
	...
	int (*flash_lock)(struct spi_flash *flash, uint32_t  ofs, size_t len);
	int (*flash_unlock)(struct spi_flash *flash, uint32_t  ofs, size_t len);
	int (*flash_is_locked)(struct spi_flash *flash, uint32_t  ofs, size_t len);
 ...
 */
int spi_flash_cmd_read(const uint8_t  *cmd, size_t cmd_len, void *data, size_t data_len);
...
int stm_lock(struct spi_flash *flash, uint32_t ofs, size_t len);

/* Unlock stmicro spi flash region */
int stm_unlock(struct spi_flash *flash, uint32_t ofs, size_t len);

/* Check if a stmicro spi flash region is completely locked */
int stm_is_locked(struct spi_flash *flash, uint32_t ofs, size_t len);
...
{
	return spi_flash_cmd(CMD_WRITE_DISABLE, NULL, 0);
}
