...
int spi_mem_exec_op(const struct spi_mem_op *op)
{
	...
	INFO("%s: cmd:%x mode:%d.%d.%d.%d addqr:%llx len:%x\n",
 ...
 * Return: 0 in case of success, a negative error code otherwise.
 */
int spi_mem_init_slave(unsigned int cs, unsigned int max_hz, int mode, const struct spi_bus_ops *ops)
{
	int ret;
	...
	spi_slave.max_hz = max_hz ?: SPI_MEM_DEFAULT_SPEED_HZ;
 ...
 * Return: 0 in case of success, a negative error code otherwise.
 */
int spi_mem_init_slave_dt(void *fdt, int bus_node, const struct spi_bus_ops *ops)
{
	int ret;
	int mode = 0;
	int nchips = 0;
