...
static inline int spi_w8r8(struct spi_slave *slave, unsigned char byte)
{
	...
	ret = spi_xfer(16, dout, din, SPI_XFER_BEGIN | SPI_XFER_END);
...
struct dm_spi_ops {
	 ...
	 */
	int (*set_speed)(struct udevice *bus, uint32_t  hz);
	 ...
	 * @return 0 if OK, -ve on error
	 */
	int (*set_mode)(struct udevice *bus, uint32_t  mode);
	 ...
	 */
	int (*cs_info)(struct udevice *bus, uint32_t  cs, struct spi_cs_info *info);
};
 ...
 *	   is invalid, other -ve value on error
 */
int spi_cs_info(struct udevice *bus, uint32_t  cs, struct spi_cs_info *info);
